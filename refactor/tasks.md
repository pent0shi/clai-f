...
- [x] **P0-04 Pinned quality toolchain** — `@vitest/coverage-v8@4.1.10`, `knip@6.33.0`, `jscpd@5.1.0`, `@stryker-mutator/core@10.0.0`, `@stryker-mutator/vitest-runner@10.0.0`, all exact, report-only. Evidence: [E-05](evidence/phase-0/README.md) — `.ts` and `.tsx` compatibility spikes for every capability; bounded mutation run produced 11 mutants (4 killed / 1 survived / 6 no-coverage) in 12 s; Stryker worker-thread `process.chdir()` limitation and `.stryker-tmp` test-discovery leak both recorded and mitigated.
