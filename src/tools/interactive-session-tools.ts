...
import { isScopeActive, loadScope, type EngagementScope } from "../store/scope.js";
import {
...
function parseInput(args: Record<string, unknown>): SessionInput {
  ...
  throw new Error("kind must be text, control, or eof");
}

/** Active engagement scope, so interactive risk matches the shell boundary. */
async function activeScope(): Promise<EngagementScope | undefined> {
  const scope = await loadScope();
  return isScopeActive(scope) ? scope : undefined;
}
