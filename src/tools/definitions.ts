...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  // --- Interactive terminal sessions (additive; legacy shell tools unchanged) ---
  def(
    "terminal.start",
    "Start a persistent interactive process or REPL you can send later input to. Returns an opaque session id and an output cursor. Use this only when a program needs multi-step interaction (python, node, psql, ssh prompts); use shell.exec for finite commands and shell.start for servers. Sessions are owned by this conversation, are never handed to the user's keyboard, and cannot be reattached after a restart.",
    {
      type: "object",
      properties: {
        command: { type: "string" },
        ...
        terminalMode: {
          ...
          description:
            "preferred (default): use a pseudoterminal when available, else pipes. required: fail with PTY_UNAVAILABLE instead of falling back. pipe: never allocate a pseudoterminal.",
        },
  ...
  def(
    "terminal.send",
    "Send one input action to an interactive session and gather the output it produces. Text is sent exactly as given plus at most one Enter. Output gathering stops on a quiet interval, the deadline, or process exit. A DEADLINE_EXCEEDED result still returns its page: continue with terminal.read from nextCursor and never resend delivered input.",
    {
      type: "object",
      properties: {
        id: { type: "string" },
        kind: { type: "string", enum: ["text", "control", "eof"] },
        text: { type: "string", description: "Required when kind is text" },
        submit: {
          type: "string",
          enum: ["enter", "none"],
          description: "enter (default) appends one Enter; none sends the bytes only",
        },
        control: {
          type: "string",
          enum: [
          ...
          description: "Required when kind is control",
        },
        cursor: {
          type: "integer",
          description: "Page start (default: the cursor captured when input was accepted)",
        },
  ...
  def(
    "terminal.read",
    "Read output from an interactive session without sending input. Pass the prior nextCursor to continue without gaps. waitMs 0 (default) returns immediately; larger values block up to 30000ms for new output. An OUTPUT_GAP result reports the earliest available cursor and the artifact holding the omitted bytes.",
    {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  def(
    "terminal.status",
    "Return the current state and process outcome of one interactive session without waiting for output.",
    {
      type: "object",
      properties: { id: { type: "string" } },
      required: ["id"],
  ...
  def(
    "terminal.list",
    "List interactive sessions owned by this conversation, live sessions first.",
    emptyObject,
    { readOnly: true },
  ),
  def(
    "terminal.resize",
    "Change the terminal dimensions of a pseudoterminal session. Pipe sessions return UNSUPPORTED_OPERATION.",
    {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  def(
    "terminal.close",
    "Close an interactive session and verify that its process tree is gone. Idempotent: closing a terminal session returns its recorded result without signalling again.",
    {
      type: "object",
      properties: {
        id: { type: "string" },
